...
object ModelTransformHelper {
    ...
    fun transformToUnitCube(asset: FilamentAsset) {
        try {
            ...
            Log.d(TAG, "Model bounds: center=($centerX, $centerY, $centerZ), maxExtent=$maxExtent, scale=$scaleFactor")
            
            // Get transform instance for root entity
            val rootInstance = transformManager.getInstance(asset.root)
            
            if (rootInstance == 0) {
                Log.w(TAG, "Root entity has no transform component, creating one")
            ...
            if (instance != 0) {
                ...
                Log.d(TAG, "Transform applied to model")
            } else {
                Log.e(TAG, "Failed to get transform instance after creation")
            }
            
        } catch (e: Exception) {
            Log.e(TAG, "Failed to transform model to unit cube", e)
        }
    }
    ...
    fun setTransform(asset: FilamentAsset, transform: FloatArray) {
        try {
            ...
            Log.e(TAG, "Failed to set transform", e)
        }
    }
    ...
    fun getTransform(asset: FilamentAsset): FloatArray? {
        return try {
            ...
            Log.e(TAG, "Failed to get transform", e)
            null
        }
    }
}
