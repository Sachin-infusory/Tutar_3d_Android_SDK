...
class FilamentResourceManager(private val context: Context) {
    ...
    fun initialize(): Boolean {
        ...
        return try {
            ...
            isInitialized = true
            Log.d(TAG, "Resource manager initialized")
            true
            
        } catch (e: Exception) {
            Log.e(TAG, "Failed to initialize resource manager", e)
            destroy()
            false
        }
    }
    ...
    private fun loadIndirectLight() {
        try {
            ...
            Log.d(TAG, "Indirect light loaded with intensity ${FilamentConfig.INDIRECT_LIGHT_INTENSITY}")
            
        } catch (e: Exception) {
            Log.e(TAG, "Failed to load indirect light: ${e.message}")
    ...
    fun applyIndirectLightToScene(scene: Scene) {
        indirectLight?.let { light ->
            ...
            Log.d(TAG, "Applied indirect light to scene")
        }
    }
    ...
    fun loadAsset(buffer: ByteBuffer): FilamentAsset? {
        ...
        return try {
            ...
            Log.d(TAG, "Asset loaded: ${asset.entities.size} entities")
            asset
    ...
    fun addAssetToScene(asset: FilamentAsset, scene: Scene) {
        ...
        Log.d(TAG, "Added ${asset.entities.size} entities to scene")
    }
    ...
    fun removeAssetFromScene(asset: FilamentAsset, scene: Scene) {
        ...
        Log.d(TAG, "Removed ${asset.entities.size} entities from scene")
    }
     ...
     */
    fun destroy() {
        Log.d(TAG, "Destroying resource manager...")
        ...
        Log.d(TAG, "Resource manager destroyed")
    }
