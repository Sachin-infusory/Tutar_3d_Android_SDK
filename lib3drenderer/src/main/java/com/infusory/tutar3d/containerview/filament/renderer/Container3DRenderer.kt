...
import com.infusory.tutar3d.containerview.filament.core.IRenderable
import com.infusory.tutar3d.containerview.filament.surface.ISurfaceCallback
import com.infusory.tutar3d.containerview.filament.surface.SurfaceManager
...
class Container3DRenderer(
    ...
    fun initialize(id: Int) {
        ...
        try {
            SdkLog.d(TAG, "[$rendererId] Initializing...")

            scene = FilamentEngineProvider.createScene()
            if (scene == null) {
                SdkLog.e(TAG, "[$rendererId] Failed to create scene")
            ...
            if (view == null) {
                SdkLog.e(TAG, "[$rendererId] Failed to create view")
            ...
            if (camera == null) {
                SdkLog.e(TAG, "[$rendererId] Failed to create camera")
            ...
            isInitialized = true
            SdkLog.d(TAG, "[$rendererId] Initialized")
            ...
            SdkLog.e(TAG, "[$rendererId] Failed to initialize", e)
    ...
    fun loadModel(buffer: ByteBuffer) {
        ...
        SdkLog.d(TAG, "[$rendererId] Loading model...")
        ...
        try {
            ...
            SdkLog.d(TAG, "[$rendererId] Model loaded")
            ...
            try {
                ...
                if (labels.isNotEmpty()) {
                    labelEntities = labels.mapNotNull { labelInfo ->
                        ...
                        if (entity != 0) {
                            SdkLog.d(TAG, "Resolved label entity: ${labelInfo.nodeName} → $entity")
                            Pair(entity, labelInfo)
                        } else {
                            SdkLog.w(TAG, "Label entity not found: ${labelInfo.nodeName}")
                            null
                        }
                    }
                }
            ...
            SdkLog.e(TAG, "[$rendererId] Load failed", e)
    ...
    override fun destroy() {
        SdkLog.d(TAG, "[$rendererId] Destroying...")
    ...
    override fun onSurfaceAvailable(surface: Surface) {
        SdkLog.d(TAG, "[$rendererId] Surface available")
        onActiveStateChanged?.invoke()
    }
    ...
    override fun onSurfaceDestroyed() {
        SdkLog.d(TAG, "[$rendererId] Surface destroyed")
        onActiveStateChanged?.invoke()
    }
