...
class LabelLineView @JvmOverloads constructor(
    ...
    // HW layer is toggled by LabelOverlayManager in show/hide so GPU buffers
    // are only allocated when labels are actually visible.
    ...
    fun commitLines() {
        // Redraw when there are lines, OR when we just dropped to zero so the
        // previously drawn lines get cleared from the canvas.
        if (lineCount > 0 || lastDrawnCount > 0) {
            invalidate()
        }
    }
