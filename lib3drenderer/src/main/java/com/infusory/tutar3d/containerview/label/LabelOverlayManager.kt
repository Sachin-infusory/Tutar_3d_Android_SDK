package com.infusory.tutar3d.containerview.label
...
class LabelOverlayManager(private val context: Context) {
    ...
    fun setLabels(newLabels: List<GlbLabelExtractor.LabelInfo>) {
        ...
        newLabels.forEach { labelInfo ->
            val tv = TextView(context).apply {
                ...
                // HW layer is applied lazily in showLabels() to avoid allocating
                // offscreen GPU buffers while labels are hidden.
                layoutParams = FrameLayout.LayoutParams(
        ...
        SdkLog.d(TAG, "Set ${entries.size} labels")
    }
    ...
    fun showLabels() {
        ...
        for (i in entries.indices) {
            entries[i].view.setLayerType(View.LAYER_TYPE_HARDWARE, null)
        }
        lineView?.setLayerType(View.LAYER_TYPE_HARDWARE, null)
    }
    ...
    fun hideLabels() {
        ...
        lineView?.clear()
        for (i in entries.indices) {
            entries[i].view.setLayerType(View.LAYER_TYPE_NONE, null)
        }
        lineView?.setLayerType(View.LAYER_TYPE_NONE, null)
    }
