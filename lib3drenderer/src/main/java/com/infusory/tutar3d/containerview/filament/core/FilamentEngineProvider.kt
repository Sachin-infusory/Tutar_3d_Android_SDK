...
object FilamentEngineProvider {
    ...
    fun initialize(): Boolean {
        ...
        return try {
            ...
            engine = Engine.create().also { eng ->
                SdkLog.d(TAG, "Engine created with backend: ${eng.backend}")
            }
            ...
            isInitialized = true
            SdkLog.d(TAG, "Filament engine initialized successfully")
            true
            
        } catch (e: Exception) {
            SdkLog.e(TAG, "Failed to initialize Filament engine", e)
            cleanup()
            false
        }
    }
    ...
    fun createSwapChain(surface: Surface, flags: Long): SwapChain? {
        return try {
            ...
            SdkLog.e(TAG, "Failed to create SwapChain with flags", e)
            null
        }
    }
    ...
    fun destroySwapChain(swapChain: SwapChain) {
        try {
            ...
            SdkLog.e(TAG, "Failed to destroy SwapChain", e)
        }
    }
...
//
    ...
    fun shutdown() {
        ...
        SdkLog.d(TAG, "Shutting down Filament engine...")
        cleanup()
        SdkLog.d(TAG, "Filament engine shutdown complete")
    }
