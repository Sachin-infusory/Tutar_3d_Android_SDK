...
class FilamentRenderLoop {
    ...
    fun register(container: Container3DRenderer): Int {
        synchronized(modificationLock) {
            ...
            SdkLog.d(TAG, "Registered container $id (total: ${containers.size})")
            return id
        }
    }
    ...
    fun unregister(id: Int) {
        synchronized(modificationLock) {
            ...
            SdkLog.d(TAG, "Unregistered container $id (total: ${containers.size})")
        }
    }
    ...
    fun start() {
        ...
        SdkLog.d(TAG, "Started - Each container has its own Renderer")
    }

    fun stop() {
        if (!isRunning.getAndSet(false)) return
        choreographer?.removeFrameCallback(frameCallback)
        SdkLog.d(TAG, "Stopped")
    }
