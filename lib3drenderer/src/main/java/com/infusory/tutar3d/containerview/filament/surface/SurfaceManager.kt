...
class SurfaceManager(
    ...
    fun initialize() {
        ...
        SdkLog.d(TAG, "Created Renderer: $renderer")
        ...
        SdkLog.d(TAG, "SurfaceManager initialized")
    }
    ...
    fun destroy() {
        SdkLog.d(TAG, "Destroying SurfaceManager")
        ...
        swapChain?.let {
            try {
                ...
                SdkLog.e(TAG, "Error destroying SwapChain", e)
            }
        }
        swapChain = null
        ...
        renderer?.let {
            try {
                ...
                SdkLog.e(TAG, "Error destroying Renderer", e)
            }
        }
        renderer = null
    ...
    private fun handleSurfaceChanged(surface: Surface) {
        SdkLog.d(TAG, "Surface changed")

        swapChain?.let {
            try {
                FilamentEngineProvider.destroySwapChain(it)
            } catch (e: Exception) {
                SdkLog.e(TAG, "Error destroying old SwapChain", e)
            }
        }
        ...
        try {
            ...
            SdkLog.d(TAG, "SwapChain created: $swapChain")
        } catch (e: Exception) {
            SdkLog.e(TAG, "Failed to create SwapChain", e)
            isReady = false
        }
    }
    ...
    private fun handleSurfaceResized(width: Int, height: Int) {
        ...
        SdkLog.d(TAG, "Surface resized: ${width}x${height}")
    }

    private fun handleSurfaceDestroyed() {
        SdkLog.d(TAG, "Surface destroyed")
        isReady = false
